...
type StoryboardScene = { scene?: string; visuals?: string; audio?: string; duration?: string; type?: string; content?: string };

function parseStoryboard(content?: string): { scenes: StoryboardScene[]; caption?: string } | null {
  if (!content?.trim()) return null;
  try {
    const parsed = JSON.parse(content.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '')) as { script?: string | StoryboardScene[]; caption?: string; captions?: string };
    if (Array.isArray(parsed.script)) {
      return { scenes: parsed.script, caption: parsed.caption || parsed.captions };
    }
  } catch { /* A plain script is handled by the clean-text fallback. */ }
  return null;
}
...
export default function ContentWorkspacePage() {
  ...
  const updateScene = (index: number, field: keyof StoryboardScene, value: string) => {
    ...
    patchDraft({ script: JSON.stringify({ script: storyboard.scenes, caption: storyboard.caption }, null, 2) });
  };
  ...
  const generateScript = async () => {
    ...
    try {
      const response = await generate({ systemPrompt: `Create a short-form production storyboard for "${draft.title || clip.title}".`, developerPrompt: `Platform: ${draft.platform || clip.platform || 'TikTok'}. Use this hook: ${draft.hook || 'create a compelling hook'}. Return a JSON object only: {"script":[{"scene":"INTRO","visuals":"what appears on screen","audio":"exact spoken words","duration":"2 seconds"}],"caption":"one optional supporting caption"}. Build 4-7 concise scenes. Keep audio natural and immediately recordable.`, taskContext: { workspace, workflowStage: 'idea', userPreferences: {}, memory: [], previousGenerations: [] }, temperature: 0.7 }, { category: 'custom', promptSummary: `Storyboard: ${clip.title}` });
      const storyboard = parseStoryboard(response.content);
      if (!storyboard) throw new Error('The AI did not return a usable storyboard. Please regenerate it.');
      patchDraft({ script: JSON.stringify({ script: storyboard.scenes, caption: storyboard.caption }, null, 2), status: draft.status === 'idea' ? 'writing' : draft.status }); setMessage('Storyboard generated. Edit any scene, then save your changes.');
  ...
  return <div className="mx-auto max-w-5xl space-y-6 animate-in fade-in duration-500">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_280px]"><div className="space-y-5"><section className="rounded-[18px] border border-white/[0.06] bg-[#111111] p-5 space-y-4"><label className="block text-[12px] text-[#71717A]">Content title<input value={draft.title ?? ''} onChange={e => patchDraft({ title: e.target.value })} className="mt-1.5 h-10 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] px-3 text-[13px] text-[#FAFAFA]" /></label><div className="flex flex-wrap gap-2"><Button onClick={() => void generateHook()} disabled={isGenerating} variant="outline" className="border-white/[0.1] bg-transparent text-[#FAFAFA]"><Sparkles className="mr-2 h-3.5 w-3.5" />Improve hook</Button><Button onClick={() => void generateScript()} disabled={isGenerating} variant="outline" className="border-white/[0.1] bg-transparent text-[#FAFAFA]"><Sparkles className="mr-2 h-3.5 w-3.5" />Generate storyboard</Button><Button onClick={() => void generateCaption()} disabled={isGenerating} variant="outline" className="border-white/[0.1] bg-transparent text-[#FAFAFA]"><Sparkles className="mr-2 h-3.5 w-3.5" />Generate caption</Button></div><label className="block text-[12px] text-[#71717A]">Hook<textarea value={draft.hook ?? ''} onChange={e => patchDraft({ hook: e.target.value })} className="mt-1.5 min-h-20 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] p-3 text-[13px] text-[#FAFAFA]" /></label><div><div className="mb-2 flex items-center justify-between"><p className="text-[12px] text-[#71717A]">Production storyboard</p>{storyboard && <span className="text-[11px] text-primary">{storyboard.scenes.length} scenes</span>}</div>{storyboard ? <div className="space-y-3">{storyboard.scenes.map((scene, index) => <article key={index} className="rounded-[14px] border border-white/[0.07] bg-[#0D0D0D] p-4"><div className="mb-3 flex items-center justify-between"><input value={scene.scene ?? `Scene ${index + 1}`} onChange={e => updateScene(index, 'scene', e.target.value)} className="w-40 bg-transparent text-[12px] font-semibold uppercase tracking-wider text-primary outline-none" /><input value={scene.duration ?? ''} onChange={e => updateScene(index, 'duration', e.target.value)} placeholder="Duration" className="w-24 rounded-full border border-white/[0.08] bg-[#161616] px-2 py-1 text-right text-[11px] text-[#A1A1AA] outline-none" /></div><label className="block text-[11px] text-[#71717A]">Spoken line<textarea value={scene.audio ?? scene.content ?? ''} onChange={e => updateScene(index, scene.audio !== undefined ? 'audio' : 'content', e.target.value)} className="mt-1 w-full min-h-20 rounded-[10px] border border-white/[0.08] bg-[#161616] p-3 text-[13px] leading-relaxed text-[#FAFAFA]" /></label><label className="mt-3 block text-[11px] text-[#71717A]">Visual direction<textarea value={scene.visuals ?? ''} onChange={e => updateScene(index, 'visuals', e.target.value)} className="mt-1 w-full min-h-16 rounded-[10px] border border-white/[0.08] bg-[#161616] p-3 text-[12px] leading-relaxed text-[#A1A1AA]" /></label></article>)}</div> : <label className="block text-[12px] text-[#71717A]">Script<textarea value={draft.script ?? ''} onChange={e => patchDraft({ script: e.target.value })} placeholder="Generate a storyboard, or write your script here…" className="mt-1.5 min-h-52 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] p-3 text-[13px] text-[#FAFAFA]" /></label>}</div><label className="block text-[12px] text-[#71717A]">Caption<textarea value={draft.caption ?? storyboard?.caption ?? ''} onChange={e => patchDraft({ caption: e.target.value })} className="mt-1.5 min-h-28 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] p-3 text-[13px] text-[#FAFAFA]" /></label><label className="block text-[12px] text-[#71717A]">CTA<input value={draft.cta ?? ''} onChange={e => patchDraft({ cta: e.target.value })} className="mt-1.5 h-10 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] px-3 text-[13px] text-[#FAFAFA]" /></label></section></div><aside className="space-y-5"><section className="rounded-[18px] border border-white/[0.06] bg-[#111111] p-5 space-y-4"><h3 className="text-[13px] font-semibold text-[#FAFAFA]">Production details</h3><label className="block text-[12px] text-[#71717A]">Platform<select value={draft.platform ?? ''} onChange={e => patchDraft({ platform: e.target.value })} className="mt-1.5 h-10 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] px-3 text-[13px] text-[#FAFAFA]"><option value="">Select platform</option>{['tiktok', 'instagram', 'youtube', 'twitter'].map(p => <option key={p} value={p}>{p}</option>)}</select></label><label className="block text-[12px] text-[#71717A]">Media URL<input value={draft.media_url ?? ''} onChange={e => patchDraft({ media_url: e.target.value, media_status: e.target.value ? 'attached' : 'missing' })} placeholder="https://…" className="mt-1.5 h-10 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] px-3 text-[13px] text-[#FAFAFA]" /></label><p className="text-[11px] leading-relaxed text-[#71717A]">Binary uploads need a configured Supabase Storage bucket. This safely records an attached media URL; it does not fake an upload.</p><label className="block text-[12px] text-[#71717A]">Schedule (local time)<input type="datetime-local" value={draft.publishing_date?.slice(0, 16) ?? ''} onChange={e => patchDraft({ publishing_date: e.target.value ? new Date(e.target.value).toISOString() : undefined })} className="mt-1.5 h-10 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] px-3 text-[13px] text-[#FAFAFA]" /></label><label className="block text-[12px] text-[#71717A]">Timezone<input value={draft.timezone ?? 'UTC'} onChange={e => patchDraft({ timezone: e.target.value })} className="mt-1.5 h-10 w-full rounded-[10px] border border-white/[0.08] bg-[#0D0D0D] px-3 text-[13px] text-[#FAFAFA]" /></label><Button onClick={() => void save('scheduled')} disabled={updateClip.isPending} className="w-full bg-primary text-white">Schedule content</Button></section><section className="rounded-[18px] border border-white/[0.06] bg-[#111111] p-5"><h3 className="text-[13px] font-semibold text-[#FAFAFA]">Ready check</h3>{readiness.length ? <p className="mt-2 text-[12px] leading-relaxed text-[#FBBF24]">Still needed: {readiness.join(', ')}.</p> : <p className="mt-2 text-[12px] text-emerald-400">Production assets complete.</p>}<p className="mt-3 text-[11px] leading-relaxed text-[#71717A]">Publication remains unavailable until an official social integration is connected. Scheduled does not mean published.</p></section></aside></div>
  </div>;
}
