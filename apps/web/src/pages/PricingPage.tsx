...
export default function PricingPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground dark:bg-[#080808] dark:text-[#FAFAFA]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_60%_32%_at_50%_8%,rgba(124,58,237,0.08),transparent_72%)] dark:bg-[radial-gradient(ellipse_60%_32%_at_50%_8%,rgba(124,58,237,0.15),transparent_72%)]" />
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_right,rgba(24,24,27,0.035)_1px,transparent_1px),linear-gradient(to_bottom,rgba(24,24,27,0.035)_1px,transparent_1px)] bg-[size:32px_32px] [mask-image:radial-gradient(ellipse_70%_62%_at_50%_20%,#000_35%,transparent_100%)] dark:bg-[linear-gradient(to_right,#ffffff03_1px,transparent_1px),linear-gradient(to_bottom,#ffffff03_1px,transparent_1px)]" />
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-20 pt-16 sm:px-6 sm:pt-24 lg:px-8 lg:pb-28">
        <motion.div initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mx-auto max-w-3xl text-center">
          <div className="mb-6 inline-flex items-center rounded-full border border-border bg-card/80 px-3 py-1 text-[12px] font-medium tracking-wide text-muted-foreground backdrop-blur dark:border-white/[0.07] dark:bg-[#111111]/80 dark:text-[#A1A1AA]">
          ...
          <p className="mx-auto mt-5 max-w-xl text-[16px] leading-relaxed tracking-tight text-muted-foreground sm:text-[18px]">Start simple. Scale when your workflow demands it.</p>
        </motion.div>

        <div className="mt-10 flex flex-col items-center gap-3 sm:mt-12">
          <div role="radiogroup" aria-label="Billing period" className="inline-grid grid-cols-2 rounded-[14px] border border-border bg-card p-1 shadow-[0_10px_30px_rgba(24,24,27,0.08)] dark:border-white/[0.08] dark:bg-[#111111] dark:shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
            {(['monthly', 'annual'] as const).map((option) => {
              ...
              }} className={`relative h-10 rounded-[10px] px-4 text-[13px] font-semibold uppercase tracking-[0.1em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background dark:focus-visible:ring-offset-[#080808] sm:px-6 ${selected ? 'text-white' : 'text-muted-foreground hover:text-foreground dark:text-[#71717A] dark:hover:text-[#A1A1AA]'}`}>
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-4 sm:mt-16 md:grid-cols-2 lg:grid-cols-3 lg:items-stretch lg:gap-5">
          {showNoUpgrades ? (
            <div className="col-span-full rounded-lg border border-border bg-card p-6 text-center text-muted-foreground dark:border-white/[0.06] dark:bg-[#0b0b0b] dark:text-[#A1A1AA]">You are on the highest plan — no upgrades available.</div>
          ) : (
            toRender.map((plan, index) => {
              ...
              return (
                <motion.article key={plan.id} initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: reduceMotion ? 0 : 0.08 * index }} whileHover={reduceMotion ? undefined : { y: -5 }} className={`relative flex min-h-[500px] flex-col overflow-hidden rounded-[22px] border p-6 transition-shadow sm:p-7 ${plan.recommended ? 'border-primary/70 bg-[linear-gradient(145deg,rgba(124,58,237,0.08),#ffffff_42%,#ffffff)] shadow-[0_12px_45px_rgba(124,58,237,0.14)] dark:bg-[linear-gradient(145deg,rgba(124,58,237,0.16),#111111_36%,#111111)] dark:shadow-[0_12px_45px_rgba(124,58,237,0.12)]' : 'border-border bg-card shadow-[0_16px_42px_rgba(24,24,27,0.08)] hover:border-primary/40 dark:border-white/[0.08] dark:bg-[#111111] dark:shadow-[0_16px_42px_rgba(0,0,0,0.15)] dark:hover:border-white/[0.14]'}`}>
                  ...
                  <div className="pt-1">
                    ...
                    <p className="mt-2 min-h-[48px] max-w-[260px] text-[15px] leading-relaxed tracking-tight text-muted-foreground dark:text-[#A1A1AA]">{plan.positioning}</p>
                  </div>
