...
function report(measurements: Measurement[]) {
  ...
  // Cost is calculated per user action, not per atomic call. This is vital when
  // production-equivalent structured-output recovery makes a second provider call.
  const rows = [...groups].map(([key, ms]) => {
    const calls = ms.flatMap(m => m.calls);
    const actionCosts = ms.map(m => m.calls.every(c => c.actualCostUsd !== undefined) ? m.calls.reduce((total, c) => total + (c.actualCostUsd ?? 0), 0) : undefined).filter((v): v is number => v !== undefined);
    const actionLatencies = ms.map(m => m.calls.reduce((total, c) => total + c.latencyMs, 0));
    const p95 = [...actionCosts].sort((a,b)=>a-b)[Math.max(0, Math.ceil(actionCosts.length*.95)-1)];
    return { operation: key, actions: ms.length, calls: calls.length, callsPerAction: (calls.length / Math.max(1, ms.length)).toFixed(2), costCoverage: `${actionCosts.length}/${ms.length}`, avgCost: dollars(average(actionCosts)), p95Cost: dollars(p95), avgLatencyMs: Math.round(average(actionLatencies) ?? 0), failureRate: `${((ms.filter(m=>!m.success).length / Math.max(1,ms.length))*100).toFixed(1)}%` };
  });
  const totalCost = all.reduce((n, c) => n + (c.actualCostUsd ?? 0), 0); const missingCosts = all.filter(c => c.actualCostUsd === undefined).length;
  const markdown = `# Creator OS AI Cost Report\n\nGenerated: ${new Date().toISOString()}\n\n## Measurement status\n\n- User operations: ${measurements.length}\n- Atomic OpenRouter calls: ${all.length}\n- Reported OpenRouter spend: ${dollars(totalCost)}${missingCosts ? ` (${missingCosts} calls did not return a cost)` : ''}\n- Success rate: ${(((all.filter(c=>c.success).length / Math.max(1,all.length))*100).toFixed(1))}%\n\n## Cost profile\n\nCosts are **summed per user action**; Calls/action includes any production-equivalent JSON retry. Cost coverage identifies actions whose every provider call returned a cost; incomplete rows are intentionally not estimated.\n\n| Feature / operation / model | Actions | Calls | Calls/action | Cost coverage | Avg cost | P95 cost | Avg latency | Failure rate |\n|---|---:|---:|---:|---:|---:|---:|---:|---:|\n${rows.map(r=>`| ${r.operation} | ${r.actions} | ${r.calls} | ${r.callsPerAction} | ${r.costCoverage} | ${r.avgCost} | ${r.p95Cost} | ${r.avgLatencyMs} ms | ${r.failureRate} |`).join('\n')}\n\n## Packaging model (assumptions, not production pricing)\n\nFor any chosen plan price, revenue required at a target margin is **AI cost / (1 - target margin)**. Set allowance from P95 cost plus a reserve for unusual output/retries and non-AI variable costs.\n`;
...
if (flag('report')) {
  const saved = JSON.parse(readFileSync(resolve(output, 'AI-COST-DATA.json'), 'utf8')) as { measurements?: Measurement[] };
  report(saved.measurements ?? []);
} else if (!flag('run')) printPlan();
